      ...
      >
          ...
          <div className="w-full h-[50%] relative flex flex-col items-center justify-center p-6">
            ...
            <div className={cn(
              ...
              {/* Stamps Grid or Points Progress Bar */}
              {template.mode === 'points' ? (
                <div className={cn(
                  "relative w-full max-w-sm transition-all px-4",
                  isCompleted ? "mb-6" : "mb-12",
                  readOnly && !onStampAddTap && !onStampRemoveTap && "pointer-events-none"
                )}>
                  <div className="flex justify-between items-end mb-2">
                    <span className="text-3xl font-extrabold tracking-tight" style={{ color: textColor.hex }}>
                      {stamps * 20} <span className="text-base font-medium opacity-80 uppercase tracking-wider">pts</span>
                    </span>
                    <span className="text-sm font-semibold opacity-80" style={{ color: textColor.hex }}>
                      Goal: {totalStamps * 20} pts
                    </span>
                  </div>
                  <div 
                    className="h-8 w-full rounded-2xl overflow-hidden shadow-inner relative" 
                    style={{ backgroundColor: stampInactiveColor.hex }}
                  >
                    <div 
                      className="h-full transition-all duration-1000 ease-out flex items-center justify-end pr-3"
                      style={{ 
                        width: `${Math.min(100, (stamps / totalStamps) * 100)}%`,
                        backgroundColor: stampActiveColor.hex
                      }}
                    >
                      {stamps > 0 && (
                        <Icon size={16} style={{ color: iconActiveColor.hex, opacity: 0.8 }} />
                      )}
                    </div>
                  </div>
                  
                  {/* Invisible tap zones for testing in editor */}
                  {(!readOnly || onStampAddTap || onStampRemoveTap) && (
                    <div className="absolute inset-0 flex z-10 cursor-pointer">
                      <div className="w-1/2 h-full" onClick={() => onStampRemoveTap ? onStampRemoveTap() : handleStampClick(stamps - 1)} />
                      <div className="w-1/2 h-full" onClick={() => onStampAddTap ? onStampAddTap() : handleStampClick(stamps)} />
                    </div>
                  )}
